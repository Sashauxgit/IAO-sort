...
public:
    ...
    dataWriter(std::string fileName):fileName(fileName + string(".csv")), entryWork(false) {
...
const dataWriter::endOfEntryLine dataWriter::endentry = dataWriter::endOfEntryLine();
...
dataWriter& operator << (dataWriter& dw, double value) {
    dw.openEntry();
...
dataWriter& operator << (dataWriter& dw, const dataWriter::endOfEntryLine& obj) {
    dw.fout << "\n";
    dw.fout.close();
    dw.entryWork = false;
    return dw;
